...
async def generate_testset(num_questions: int = 10):
    ...
    llm = ChatOpenAI(
        model="gpt-4o", temperature=0.3, api_key=SecretStr(settings.OPENAI_API_KEY)
    )
    ...
    prompt = ChatPromptTemplate.from_messages(
        [
            (
                "system",
                "You are an expert software engineer and AI researcher. Use the provided code context to generate a realistic question and a precise ground truth answer. The question should be something a developer would actually ask about this codebase. The answer must be strictly based on the provided context.",
            ),
            (
                "human",
    ...
    for idx in sampled_indices:
        ...
        print(
            f"  - Generating triplet for: {meta.get('symbol_name', meta.get('source_id'))}"
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Generate a synthetic test set for RAG evaluation."
    )
    parser.add_argument(
        "--num_questions", type=int, default=10, help="Number of questions to generate."
    )
