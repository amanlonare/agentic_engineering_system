...
class IngestionManager:
    ...
    async def ingest_remote_repo(self, repo_url: str, repo_name: Optional[str] = None, deep_index: bool = True) -> str:
        """
        ...
        if deep_index:
            logger.info("Starting deep indexing of %s via ephemeral clone...", repo_name)
            try:
                local_path = await self.resource_manager.ensure_local_context(mcp_prefix)
                    ...
                    for file in files:
                        ...
                        if ext in supported_exts:
                            ...
                            try:
                                ...
                                file_chunks = chunker.chunk(content, source_id=file_mcp_uri, chunk_format=format_name)
                                for c in file_chunks:
                                    if not c.metadata.custom_attributes:
                                        c.metadata.custom_attributes = {}
                                    c.metadata.custom_attributes["repo_name"] = repo_name
                                    c.metadata.custom_attributes["mcp_uri"] = file_mcp_uri
                                    
                                all_chunks.extend(file_chunks)
                            except Exception as e:
                                logger.debug("Deep indexing failed on %s: %s", relative_path, e)
                
                if all_chunks:
                    v_store = VectorStore()
                    v_store.upsert_chunks(all_chunks)
                    logger.info("Successfully deep-indexed %d chunks for %s.", len(all_chunks), repo_name)
    ...
    async def ingest_gdrive_folder(self, folder_id: str, folder_name: str) -> str:
        ...
        self.graph_store.conn.execute(
            ...
            {"name": folder_name, "url": f"https://drive.google.com/drive/folders/{folder_id}", "mcp": mcp_uri}
        )
...
if __name__ == "__main__":
    ...
    async def test():
        ...
        # await manager.ingest_remote_repo("https://github.com/amanlonare/agentic_engineering_system")
