...
class ExecuteRequest(BaseModel):
    ...
    sources: List[SourceItem] = Field(
        default_factory=list, description="List of sources to act on."
    )
    repo_url: Optional[str] = Field(
        None, description="Legacy field for single repo URL."
    )

    # Mode & Model Selection
    mode: str = Field(
        "standard", description="'standard' (defaults) or 'custom' (user keys)."
    )
...
async def event_generator(request_data: ExecuteRequest) -> AsyncGenerator[str, None]:
    ...
    try:
        ...
        trigger = TriggerContext(
            type="manual",
            repo_name=request_data.sources[0].url.split("/")[-1].replace(".git", "")
            if request_data.sources
            else "generic",
            payload={
                "query": request_data.query,
                "sources": [s.dict() for s in request_data.sources],
            },
        )
...
async def execute_task(request: ExecuteRequest):
    ...
    """
    logger.info(
        f"Received execution request. Mode: {request.mode}, Query: {request.query[:50]}..."
    )
...
async def get_ingested_sources():
    ...
    repos = wm.graph_store.execute_query(
        "MATCH (r:Repository) RETURN r.name, r.remote_url"
    )

    sources = []
    for r in repos:
        sources.append({"id": r[0], "url": r[1], "type": "repo"})
...
async def get_configured_sources():
    ...
    try:
        ...
        for url in config.get("repositories", []):
            sources.append(
                {"id": f"config-{url.split('/')[-1]}", "url": url, "type": "repo"}
            )
        # Parse documents
        for url in config.get("google_docs", []):
            sources.append(
                {"id": f"config-{url.split('/')[-1]}", "url": url, "type": "doc"}
            )
