...
def main():
    ...
    parser.add_argument("--limit", type=int, default=50, help="Limit the number of results (default: 50)")
    ...
    query += " RETURN r.name, d.path, c.symbol_name, c.chunk_type LIMIT " + str(args.limit)
    ...
    try:
        ...
        print(f"{'REPOSITORY':<25} | {'DOCUMENT PATH':<40} | {'SYMBOL NAME':<30} | {'TYPE'}")
        print("-" * 110)
        for row in results:
