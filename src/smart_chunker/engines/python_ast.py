...
class PythonEngine(BaseEngine):
    """
    Code chunking engine for Python using Tree-sitter.
    ...
    def chunk(self, content: str, source_id: str, **_kwargs) -> List[Chunk]:
        ...
        for node, tag in captures:
            if tag in ["class", "func"] and node.id not in processed_nodes:
                ...
                # Metadata extraction
                symbol_name = self._get_node_text(
                    node.child_by_field_name("name"), content
                )
                signature = content[
                    node.start_byte : node.children[-1].start_byte
                ].split("\n")[0]
    ...
    def _extract_dependencies(self, node, content: str, self_name: str) -> List[str]:
        """Simple extraction of identifiers used in the node that might be dependencies."""
        ...
        for n, _ in captures:
            ...
            if text and text[0].isupper() and text != self_name:
                deps.add(text)
