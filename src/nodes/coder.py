...
async def coder_node(state: EngineeringState, config: RunnableConfig) -> Dict[str, Any]:
    ...
    if state.task_plan and state.active_step_id:
        current_step = next((s for s in state.task_plan.steps if s.id == state.active_step_id), None)

    if not current_step and state.task_plan:
        # Fallback to first uncompleted coder step
        current_step = next((s for s in state.task_plan.steps if s.assigned_to == "coder" and s.id not in (state.completed_step_ids or [])), None)
    ...
    repo_name = current_step.target_repo or (state.trigger.repo_name if state.trigger and state.trigger.repo_name else "unknown")
    ...
    instructions = f"Goal: {current_step.description}\n\nAdditional Context/Feedback: {last_msg}"
